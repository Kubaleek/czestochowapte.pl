...
import { usePageData } from "@/lib/getPages";
...
export default function SlugMain({ slug }: { slug: string }) {
  ...
  const pageData = data?.pages.nodes || [];

  const pageSlug = pageData.find((item) => path.includes(item.slug));
    ...
    <PagesLayout>
      <div className="grid grid-cols-12">
        ...
        {loading ? null : error ? null : (
          <section className="col-span-12 md:col-span-8 xl:col-span-9 flex flex-col gap-6 py-[40px] px-[20px]">
            <div className="flex flex-col gap-2">
              <div>
                <h2 className="text-lg font-bold text-[#17822e] break-words">{pageSlug?.title}</h2>
                <h1 className="text-xl md:text-3xl font-bold">{pageSlug?.title}</h1>
              </div>
            ...
            <div
              className="content break-all flex flex-col gap-3"
              dangerouslySetInnerHTML={{ __html: pageSlug?.content || "" }}
            />
          </section>
        )}
      </div>
